...
-- TulasiAI Labs - Production SaaS Database
-- ========================================

-- Migration Info
-- Version: 001
-- Description: Initial database schema with all required tables
-- Dependencies: None
-- Rollback: Drop all tables and recreate
...
SELECT 
    ...
    COALESCE(SUM(s.hours_practiced), 0) as total_hours_practiced,
...
GROUP BY p.id, p.name, p.email, st.current_streak, p.job_readiness_score;
...
SELECT 
    ...
    '{"migration": "001_initial_schema", "timestamp": "' || NOW() || '"}' as activity_data
FROM profiles
LIMIT 1;
